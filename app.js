...
function getPresenceStamp(presence) {
  return Number(presence?.updatedAt || presence?.lastSeen || presence?.lastChanged || 0);
}

function isPresenceFresh(presence) {
  return Boolean(presence?.online) && getPresenceStamp(presence) > 0 && (Date.now() - getPresenceStamp(presence)) < ONLINE_FRESH_MS;
}
...
function updateIdentity() {
  ...
  const peerOnline = isPresenceFresh(peerPresence);
  const peerLastSeenValue = Number(peerPresence.lastSeen || peerPresence.updatedAt || peerPresence.lastChanged || 0);
  ...
  els.peerStatusLabel.textContent = peer
    ? typingNow
      ...
      : peerOnline
        ? `${peer.name} متصل الآن`
        : peerLastSeenValue
          ? `آخر ظهور: ${formatLastSeen(peerLastSeenValue)}`
          : `${peer.name} غير متصل`
    : "—";
...
function detachChannel() {
  ...
  if (state.presenceConnectedRef && listeners) state.presenceConnectedRef.off("value", listeners.connected);

  if (state.presenceHeartbeatTimer) clearInterval(state.presenceHeartbeatTimer);
  ...
  state.presenceConnectedRef = null;
...
function setPresence(extra = {}) {
  ...
  return state.presenceSelfRef.update({
    ...
    typingAt: state.inputTyping ? serverTimestamp() : null,
    name: user.name,
    ...extra
  }).catch(() => {});
}
...
function markTyping(active) {
  ...
  setPresence({ typing: active, typingAt: active ? serverTimestamp() : null });
  if (state.typingTimer) clearTimeout(state.typingTimer);
  if (active) {
...
function attachChannel() {
  ...
  state.presenceConnectedRef = state.db.ref(".info/connected");
  state.ready = true;
  ...
  state.listeners = {
    ...
    connected: (snap) => {
      ...
      state.presenceSelfRef.onDisconnect().update({
        online: false,
  ...
  state.presenceConnectedRef.on("value", state.listeners.connected);
...
async function sendMessage(text) {
  ...
  state.replyTo = null;
  renderReplyBar();
  await setPresence({ typing: false, typingAt: null });
}
